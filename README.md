...
1. 扩写尺度（详实且成体系）：
   ...
   - 严禁“车轱辘话”和空泛的形容词。必须补充有逻辑承载力的学术信息（例如：详细拆解公式中各个变量的数学/物理意义、深入剖析算法模块设计的前提假设与约束条件、详述实验现象背后的深层机理、全面揭示对比基准的局限性）。
...
4. 输出格式（严格分区）：
   - Part 1 [Expanded LaTeX]：只输出扩写与润色后的中文 LaTeX 代码本身，不加任何非 LaTeX 标记。
